...
from ..sei_proto.cosmos.crypto.ed25519 import PubKey as ValConsPubKey_pb
from ..sei_proto.cosmos.crypto.multisig import LegacyAminoPubKey as LegacyAminoPubKey_pb
from ..sei_proto.cosmos.crypto.secp256k1 import PubKey as SimplePubKey_pb
...
class SimplePublicKey(PublicKey):
    ...
    def address(self) -> str:
        return get_bech("sei", self.raw_address().hex())
...
class ValConsPubKey(PublicKey):
    ...
    def address(self) -> str:
        return get_bech("seivalcons", self.raw_address().hex())
...
class LegacyAminoMultisigPublicKey(PublicKey):
    ...
    def from_amino(cls, amino: dict) -> LegacyAminoMultisigPublicKey:
        return cls(
            ...
            public_keys=[
                SimplePublicKey.from_amino(pubkey) for pubkey in amino["value"]["public_keys"]
            ],
        )
    ...
    def address(self) -> str:
        address = get_bech("sei", self.raw_address())
        return address

    def pubkey_address(self) -> str:
        return get_bech("seipub", str(self.encode_amino_pubkey()))
