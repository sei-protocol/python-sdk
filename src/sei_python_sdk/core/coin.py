...
import attr
from ..sei_proto.cosmos.base.v1beta1 import Coin as Coin_pb
...
class Coin(JSONSerializable):
    ...
    in Cosmos SDK. Used for representing Sei native assets.
    """

    denom: str = attr.ib()
    """Coin's denomination, only ``usei``."""
    ...
    def from_str(cls, string: str) -> Coin:
        ...
        ``283923usei`` (``int``-Coin) or ``23920.23020usei`` (:class:`Dec`-Coin).

        >>> int_coin = Coin.from_str("230920usei")
        >>> int_coin.denom
        'usei'
        >>> int_coin.amount
        230920
        >>> dec_coin = Coin.from_str("203922.223usei")
        >>> dec_coin.denom
        'usei'
        >>> dec_coin.amount
        Dec('203922.223')
