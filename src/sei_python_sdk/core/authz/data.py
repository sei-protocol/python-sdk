...
from ...sei_proto.cosmos.authz.v1beta1 import (
    GenericAuthorization as GenericAuthorization_pb,
)
from ...sei_proto.cosmos.authz.v1beta1 import Grant as Grant_pb
from ...sei_proto.cosmos.bank.v1beta1 import SendAuthorization as SendAuthorization_pb
from ...sei_proto.cosmos.staking.v1beta1 import AuthorizationType
from ...sei_proto.cosmos.staking.v1beta1 import (
...
from sei_python_sdk.core import AccAddress, Coin, Coins
from sei_python_sdk.util.base import BaseSeiData
from sei_python_sdk.util.converter import to_isoformat
...
class Authorization(BaseSeiData):
    """Base class for authorization types."""

    @staticmethod
    def from_amino(amino: dict) -> Authorization:
        from sei_python_sdk.util.parse_authorization import parse_authorization_amino

        return parse_authorization_amino(amino)

    @staticmethod
    def from_data(data: dict) -> Authorization:
        from sei_python_sdk.util.parse_authorization import parse_authorization

        return parse_authorization(data)

    @staticmethod
    def from_proto(proto: Any_pb) -> Authorization:
        from sei_python_sdk.util.parse_authorization import parse_authorization_proto

        return parse_authorization_proto(proto)

    @staticmethod
    def unpack_any(proto: Any_pb) -> Authorization:
        from sei_python_sdk.util.parse_authorization import parse_authorization_unpack_any
...
class SendAuthorization(Authorization):
    """Type of :class:`Authorization` for :class:`MsgSend<sei_python_sdk.core.bank.msgs.MsgSend>`,
...
class StakeAuthorization(Authorization):
    ...
    def from_data(cls, data: dict) -> StakeAuthorization:
        return StakeAuthorization(
            ...
            max_tokens=(
                Coin.from_data(data["max_tokens"]) if data.get("max_tokens") is not None else None
            ),
